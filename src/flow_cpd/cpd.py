...
def select_main_kernel(
    variance_1: float = 1.0,
    lengthscale_1: float = 1.0,
    variance_2: float = 1.0,
    lengthscale_2: float = 1.0,
    use_sum: bool = True,
):
    """Build the needed kernel.
    ...
    Returns:
        Constructed gpflow.kernel
    """
    if use_sum:
        kernel_1 = gpflow.kernels.Matern12(
...
def fit_matern_kernel(
    ...
    m = gpflow.models.GPR(
        ...
        kernel=select_main_kernel(
            variance_1=variance,
            lengthscale_1=lengthscale,
            variance_2=variance,
            lengthscale_2=lengthscale,
            use_sum=use_sum,
        ),
    ...
    if use_sum:
        ...
        param_catcher = m.kernel

    params = {
        "kM_variance": np.sum([var.variance.numpy() for var in param_catcher]),
        "kM_lengthscales": np.max(
            [length.lengthscales.numpy() for length in param_catcher]
        ),
...
def fit_changepoint_kernel(
    ...
    if use_sum:
        ...
        param_catcher_1 = [m.kernel.kernels[0]]
        param_catcher_2 = [m.kernel.kernels[1]]

    changepoint_location = m.kernel.locations[0].numpy()
    params = {
        "k1_variance": np.sum([var.variance.numpy() for var in param_catcher_1]),
        "k1_lengthscale": np.max(
            [length.lengthscales.numpy() for length in param_catcher_1]
        ),
        "k2_variance": np.sum([var.variance.numpy() for var in param_catcher_2]),
        "k2_lengthscale": np.max(
            [length.lengthscales.numpy() for length in param_catcher_2]
        ),
...
def changepoint_loc_and_score(
    ...
    # Rescale all the pct_change data
